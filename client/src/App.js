...
import thermometerImage from './assets/thermometer_2903595 - Copy.png';
    ...
    try {
     const response = await axios.get(`https://project-weather-omej.onrender.com/weather?city=${city}`);
      ...
      if (response.data.weather && Array.isArray(response.data.weather)) {
        // Slice to get the next 4 days (starting from index 1 to skip today)
        const nextFourDays = response.data.weather.slice(1, 5); // Get next 4 days
        setUpcomingWeather(nextFourDays);
      } else {
    ...
    <div className="App">
      ...
      {!weather && !loading && !error && (
        <div className="image-placeholder">
          <img src={thermometerImage} alt="Weather Placeholder" />
        </div>
      )}
